...
HELPER_SRC="$SYSTEMD_SRC/src/libsystemd/sd-journal/test-fss-vector-generator.c"
...
run cp "$SCRIPT_DIR/fsprg_vector_generator.c" "$HELPER_SRC"
...
text = path.read_text()
entry = """        {
                'sources' : files('sd-journal/test-fss-vector-generator.c'),
                'type' : 'manual',
        },
"""
if "sd-journal/test-fss-vector-generator.c" not in text:
    marker = """        {
...
run ninja -C "$SYSTEMD_BUILD" test-fss-vector-generator
printf '%s\n' "$SYSTEMD_BUILD/test-fss-vector-generator"
