...
- `SOW-0104-20260611-python-explorer-netdata-parity.md` - in-progress. Activated after SOW-0103 completed; gate refreshed with a fresh API-diff inventory. Ports Explorer, the Netdata function API, the stdin wrapper, and source selector labels to Python; joins the Netdata comparator matrices as third peer; adds `pyproject.toml` (no publication) and the missing facade unique-values visitor.
