...
- `SOW-0108-20260614-cross-language-reader-window-accessor.md` - completed. Rust remains the mmap/windowing reference; Go and Python now expose bounded rolling reader access with mmap/read-at selection; Node.js production reader paths no longer use whole-file resident Buffers and now use bounded positioned-read windows, row-pinned current-row views, row-arena memory, and streaming `.journal.zst` temp files. Go passed production review in round 3, Python in round 2, and Node.js in round 3 with all six reviewers voting `PRODUCTION GRADE`.
